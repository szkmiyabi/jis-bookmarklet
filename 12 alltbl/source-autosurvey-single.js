javascript:(function(){
		...
		H2_PDF_ICON_PASS2: function() {
			...
			for(var i=0; i<arr.length; i++) {
				...
				if(is_mic_doc_link(diag.clean_text(description)) && is_mic_doc_same_alt(diag.clean_text(description))) {
					...
					diag.set_text(comment_obj, "この実装番号は満たすが、のリンクテキストがリンクアイコンのalt属性値と重複する。");
				}
			}
			diag.save();
	...
	switch(tech) {
		case "H2":
			actName = "H2_PDF_ICON_PASS2";
			break;
		case "H30":
		case "IFC1015":
    	case "G53":
			actName = "H30_PDF_ICON_FAIL";
			break;
	}
