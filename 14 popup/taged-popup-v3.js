javascript:(function() {
	/*comments*/var cmt_str = "対象ソースコードに示した文字列は、段落の体裁を取っているが、p要素でマークアップされていない|リストの視覚表現を含んだコンテンツが、箇条書き要素としてマークアップされていない|対象ソースコードに示した文字列は、見出しの体裁を取っているが、見出し要素としてマークアップされていない|強調を意図しているなら、セマンティックなマークアップが必要|強調を意図する箇所でセマンティックな要素を用いていない(7131_IFC2001参照)";
	...
	function trgr() {
		...
		my_html += '<button onclick="do_numbering()">Num</button>';
		my_html += '<button onclick="do_submit()">Save</button>';
		my_html += '<br>';
		my_html += '<select id="sv_ctrl" onchange="do_sv_select()"></select>';
		...
		my_html += '<label>CSS</label><select id="prop_ctrl"></select><button onclick="this_ctrl_insert(\'prop_ctrl\')">挿入</button>';
		my_html += '<br>';
		...
		if(document.getElementById("mycss") == null) {
			...
			mycss_str += '#sv_ctrl { width: 90px; }';
			mycss_str += '#comment_ctrl { width: 230px; }';
			mycss_str += '#tag_ctrl {width:120px;} #attr_ctrl {width:50px;} #prop_ctrl {width:50px;}';
		...
		if(document.getElementById("myjs") == null) {
			...
			myjs_str += '}';
			myjs_str += 'function do_sv_select() {';
			myjs_str += 'var sv_ctrl = document.getElementById("sv_ctrl");';
			myjs_str += 'var sv_flag = sv_ctrl.options[sv_ctrl.selectedIndex].value;';
			myjs_str += 'if(sv_flag != "") {';
			myjs_str += 'var form = document.forms["diag_form"];';
			myjs_str += 'for(var i=0; i<form.elements.length; i++) {';
			myjs_str += 'var obj = form.elements[i].value;';
			myjs_str += 'if(obj == sv_flag) {';
			myjs_str += 'form.elements[i].click();';
			myjs_str += 'break;';
			myjs_str += '}';
			myjs_str += '}';
			myjs_str += '}';
			myjs_str += '}';
			myjs_str += 'function do_submit() {';
			myjs_str += 'var form=document.forms["diag_form"];';
			myjs_str += 'for(var i=0;i<form.elements.length;i++){';
			myjs_str += 'var obj=form.elements[i].type;';
			myjs_str += 'if(obj=="submit"){';
			myjs_str += 'form.elements[i].click();';
			myjs_str += '}';
			myjs_str += '}';
			myjs_str += '}';
			myjs_str += 'function do_numbering() {';
			myjs_str += 'var comment = document.getElementById("comments").value;';
			myjs_str += 'var description = document.getElementById("description").value;';
			myjs_str += 'var srccode = document.getElementById("srccode").value;';
			myjs_str += 'comment = "1." + comment + "\\n2.";';
			myjs_str += 'description = "1.\\n" + description + "\\n2.\\n";';
			myjs_str += 'srccode = "1.\\n" + srccode + "\\n2.\\n";';
			myjs_str += 'document.getElementById("comments").value = comment;';
			myjs_str += 'document.getElementById("description").value = description;';
			myjs_str += 'document.getElementById("srccode").value = srccode;';
			myjs_str += '}';
